...
void prn(const unsigned char x[X448_BYTES]) {
    ...
    for (i=0; i<X448_BYTES; i++) {
        printf("%02x", x[i]);
        if (i==28-1 || i==56-1) printf("\n");
    }
}

int expect(
    const unsigned char got[X448_BYTES],
    const unsigned char answer[X448_BYTES]
) {
    if (memcmp(got,answer,X448_BYTES)) {
        printf("You fail! Got:\n");
        prn(got);
        printf("Expected:\n");
        prn(answer);
...
int main(int argc, char **argv) {
    ...
    unsigned char sa[X448_BYTES],sb[X448_BYTES],
        pa[X448_BYTES],pb[X448_BYTES],
        ab[X448_BYTES],ba[X448_BYTES],
        out[X448_BYTES];
    

    int ret = 0;
    ret |= x448(out,scalar1,base1);
    ret |= expect(out,answer1);

    ret |= x448(out,scalar2,base2);
    ret |= expect(out,answer2);
    
    ret |= x448_base(out,alicePriv);
    ret |= expect(out,alicePub);
    ret |= x448_base(out,bobPriv);
    ret |= expect(out,bobPub);
    ret |= x448(out,bobPriv,alicePub);
